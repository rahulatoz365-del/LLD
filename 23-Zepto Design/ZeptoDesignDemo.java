...
// Inventory Store and Db Store (Interfaces)
interface InventoryStore {
    void removeProduct(int sku,int quantity);
    void addProduct(Product product,int quantity);
    int checkStocks(int sku);
    List<Product> listAllProducts();
}
class DbInventoryStore implements InventoryStore {
    private Map<Integer,Product> products = new HashMap<>(); // SKU -> Product
    private Map<Integer,Integer> stocks = new HashMap<>(); // SKU -> Quantity
    @Override
    public void addProduct(Product product,int quantity) {
        ...
        if(!products.containsKey(sku)) {
            ...
            stocks.put(sku,stocks.get(sku)+quantity);
        }
    }
    @Override
    public void removeProduct(int sku,int quantity) {
        ...
        int currentStock = stocks.get(sku);
        int remainingStock = currentStock-quantity;
        if(remainingStock > 0) {
            stocks.put(sku,remainingStock);
        }
        else {
            stocks.remove(sku);
            products.remove(sku);
    ...
    @Override
    public int checkStocks(int sku) {
    ...
    public List<Product> listAllProducts() {
        ...
        for(Map.Entry<Integer,Integer> it:stocks.entrySet()) {
            ...
            if(quantity>0 && products.containsKey(sku)) {
                productsList.add(products.get(sku));
            }
        }
        return productsList;
    }
...
class InventoryManager{
    ...
    public void addStock(int sku,int quantity) {
        ...
        System.out.println("[InventoryManager] Added "+quantity+" "+ProductFactory.createProduct(sku).getName()+" to inventory.");
    }
    public void removeStock(int sku,int quantity) {
        store.removeProduct(sku,quantity);
        System.out.println("[InventoryManager] Removed "+quantity+" "+ProductFactory.createProduct(sku).getName()+" from inventory.");
    }
    public int checkStock(int sku){
        return store.checkStocks(sku);
    }
...
// Zepto Initialization and Main Class
class ZeptoHelper {

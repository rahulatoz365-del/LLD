...
Based on your notes, here is how a Car class translates from code to a UML structure:
```text
+------------------------------------------+
|                   Car                    |  <-- Class Name
+------------------------------------------+
| + brand : String                         |  <-- Attributes / Variables
| + model : String                         |
| + engineCC : int                         |
+------------------------------------------+
| - startEngine() : void                   |  <-- Methods / Behaviors
| - stopEngine() : void                    |
| - accelerate() : void                    |
| - brake() : void                         |
+------------------------------------------+
 ...
 * **Symbol**: An open arrow pointing from the child class to the parent class (\longrightarrow or \vartriangle).
 ...
 * **Symbol**: An open/hollow diamond (\diamondsuit) on the side of the parent/container class.
 ...
 * **Symbol**: A filled/solid diamond (\blacklozenge) on the side of the parent/owner class.
...
### 🧱 Core Components of a Sequence Diagram
 ...
 2. **Lifeline**: A dashed vertical line representing the existence of an object over time.
 3. **Activation Bar**: A thin vertical rectangle on the lifeline showing when an object is actively performing an operation.
 ...
 * **Synchronous (Sync)**: The sender blocks and waits for a response before proceeding.
   * *Representation*: Solid arrow line with a filled arrowhead (\longrightarrow).
 * **Asynchronous (Async)**: The sender sends the message and continues working immediately without waiting.
   * *Representation*: Solid arrow line with an open arrowhead (\rightarrow).
 * **Return Message**: Sends back data to the caller.
   * *Representation*: Dashed line with an arrow (\dashrightarrow).
## 4. Real-World Case Study: ATM Cash Withdrawal
Let's model an ATM Cash Withdrawal sequence to bring it all together.
### Use Case Context
 ...
 * **alt (Alternatives / If-Else)**: Used when only one of multiple paths can be taken (e.g., alt Balance \ge Amount vs Balance < Amount).
